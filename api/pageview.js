...
async function isAlreadyCounted(ip) {
    ...
    const res = await fetch(`${url}/get/${seenKey}`, {
        headers: { Authorization: `Bearer ${token}` },
    })
    const json = await res.json()
    if (json.result) return true // already seen

    // Mark as seen for 25 hours (covers timezone edge cases)
...
// Simple in-memory cooldown so a single IP can't spam the endpoint
...
export default async function handler(req, res) {
    ...
    try {
        ...
        let total = null
        if (hasKV) {
            total = await kvIncr(VISIT_TOTAL_KEY)
        }
